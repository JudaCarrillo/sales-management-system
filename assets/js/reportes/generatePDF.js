...
function generatePDFBills(result, title) {
  const { jsPDF } = window.jspdf;
  const doc = new jsPDF();
  ...
  doc.rect(margin, startY, 190, 10, "F");
  doc.setTextColor(0, 0, 0);
  doc.text("Cliente", margin + 2, startY + 7);
  doc.text("Fecha", margin + 30, startY + 7);
  doc.text("Divisa", margin + 70, startY + 7);
  doc.text("IGV", margin + 110, startY + 7);
  doc.text("Importe Total", margin + 150, startY + 7);
  startY += 10;
  ...
  result.forEach((row) => {
    doc.rect(margin, startY, 190, 10); // Dibujar rectángulo alrededor de cada fila
    doc.text(row.name, margin + 2, startY + 7);
    doc.text(row.date_issue.split(" ")[0], margin + 30, startY + 7);
    doc.text(row.currency, margin + 70, startY + 7);
    doc.text(row.igv, margin + 110, startY + 7);
    doc.text(row.final_price, margin + 150, startY + 7);
    startY += 10;
  });
